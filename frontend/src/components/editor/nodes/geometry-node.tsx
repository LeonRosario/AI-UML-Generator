...
import { InlineLabel, NodeHandles, nodeStyle, resolveTextStyle } from './shared';
import { ActorFigure } from './actor-node';

const diamond = new Set(['flowDecisionNode','erRelationshipNode','activityDecisionNode','stateChoiceNode','decisionNode','constraintNode']);
const ellipse = new Set(['erAttributeNode','sequenceEntityNode','useCaseNode','signalNode']);
const cylinder = new Set(['flowDatabaseNode','erDatabaseNode','erTableNode','archServerNode','cloudStorageNode','networkServerNode','databaseNode','dataStoreNode']);
const circle = new Set(['flowConnectorNode','activityInitialNode','stateInitialNode','portNode','commentNode']);
const final = new Set(['activityFinalNode','stateFinalNode']);
const pill = new Set(['flowTerminatorNode','activityActionNode','stateNode','messageNode','returnMessageNode','selfMessageNode','createMessageNode','destroyMessageNode']);
const para = new Set(['flowInputOutputNode','dataFlowNode']);
const documentShape = new Set(['flowDocumentNode','artifactNode','noteNode','commentNode']);
const cloud = new Set(['networkCloudNode','archCloudNode','cloudNetworkNode']);
const actor = new Set(['sequenceActorNode','networkClientNode','archClientNode','externalEntityNode']);
const packageLike = new Set(['packageNode','componentNode','interactionNode','processNode','dfdProcessNode','boundaryNode','controlNode','systemBoundaryNode']);
...
function GeometryNodeComponent({ id, data, selected }: NodeProps<Node<DiagramNodeData>>) {
  ...
  const labelEl = <foreignObject x="14" y="35" width="132" height="48"><div className="flex h-full items-center justify-center px-2 text-center text-[12px] font-medium break-words" style={{ ...resolveTextStyle(data), color: String(data.textColor ?? '#0f172a') }}><InlineLabel value={label} onRename={(value) => update(id, { label: value })} /></div></foreignObject>;
  ...
  if (packageLike.has(type)) return <div className={`uml-node relative h-[92px] w-[160px] ${selected ? 'ring-2 ring-indigo-400' : ''}`}><NodeHandles /><svg viewBox="0 0 160 92" className="h-full w-full overflow-visible"><path d="M16 18V12h66l10 6v52H16z" fill={String(data.fill ?? '#fff')} stroke={color} strokeWidth="2" /><path d="M16 18h76l-10-6" fill="none" stroke={color} strokeWidth="2" /><path d="M16 34h76" stroke={color} strokeWidth="2" /></svg>{labelEl}</div>;
  return <div className={`uml-node relative h-[92px] w-[160px] ${selected ? 'ring-2 ring-indigo-400' : ''}`}><NodeHandles />
    <svg viewBox="0 0 160 92" className="h-full w-full overflow-visible">
      ...
      {!diamond.has(type)&&!ellipse.has(type)&&!para.has(type)&&!pill.has(type)&&!documentShape.has(type)&&!cylinder.has(type)&&!cloud.has(type)&&!packageLike.has(type) && <rect x="2" y="2" width="156" height="88" rx={type === 'flowProcessNode' ? 0 : 8} fill={String(data.fill ?? '#fff')} stroke={color} strokeWidth="2" />}
      {labelEl}
    </svg></div>;
}
