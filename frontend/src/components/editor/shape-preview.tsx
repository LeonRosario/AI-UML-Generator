...
const diamonds = new Set(['flowDecisionNode', 'erRelationshipNode', 'activityDecisionNode', 'stateChoiceNode']);
const ellipses = new Set(['useCaseNode', 'erAttributeNode', 'sequenceEntityNode']);
const cylinders = new Set(['databaseNode', 'flowDatabaseNode', 'erDatabaseNode', 'erTableNode', 'archServerNode', 'cloudStorageNode', 'networkServerNode']);
const pills = new Set(['flowTerminatorNode', 'activityActionNode', 'stateNode']);
const documents = new Set(['flowDocumentNode', 'artifactNode']);
const clouds = new Set(['networkCloudNode', 'archCloudNode', 'cloudNetworkNode']);
const actors = new Set(['actorNode', 'sequenceActorNode', 'networkClientNode', 'archClientNode']);
...
export function ShapePreview({ type, className = '' }: ShapePreviewProps) {
  ...
  if (type === 'classNode' || type === 'interfaceNode' || type === 'abstractClassNode' || type === 'objectNode') return <svg aria-hidden="true" viewBox="0 0 96 52" className={className}><rect x="15" y="3" width="66" height="46" rx="1" fill="white" stroke={stroke} strokeWidth="2" /><path d="M15 18h66M15 34h66" stroke={stroke} strokeWidth="2" /></svg>;
  ...
  return <svg aria-hidden="true" viewBox="0 0 96 52" className={className}>
    ...
    {!diamonds.has(type) && !ellipses.has(type) && type !== 'flowInputOutputNode' && !pills.has(type) && !documents.has(type) && !cylinders.has(type) && !clouds.has(type) && type !== 'activityForkNode' && <rect x="8" y="5" width="80" height="42" rx={type === 'flowProcessNode' ? 0 : 4} fill="white" stroke={stroke} strokeWidth="2" />}
  </svg>;
}
